...
largest db 127              ; Variable to store the largest number
...
find_largest:
    ...
    cmp al, largest         ; Compare the current element with the largest number
    jbe skip_update         ; If the current element is not larger, skip the update

    mov largest, al         ; Update the largest number with the current element
...
skip_update:
    ...
    add largest, '0'        ; Convert the largest number to ASCII
    mov dl, largest         ; Move the ASCII character to DL
